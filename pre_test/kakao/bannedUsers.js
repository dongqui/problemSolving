function solution(user_id, banned_id) {
  const bannedList = 
        new Array(banned_id.length).fill().map(v => new Array());
  for (const [ i, b ] of banned_id.entries()) {
  ...
  let answer = [];
  function recur(idx, list) {
      if (idx > bannedList.length - 1) {
          if (list.length === banned_id.length
              && !isDupli(answer, list)) answer.push(list);
          return;
      }
      for (let i = 0; i < bannedList[idx].length; i++) {
          list.includes(bannedList[idx][i]) || 
              recur(idx + 1, [...list ,bannedList[idx][i]]); 
      }
  }
  recur(0, []);
...
function isDupli(answer, newOne) {
