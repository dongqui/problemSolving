function solution(str1, str2) {
  let s1 = splitStr(str1);
  let s2 = splitStr(str2);
  if (!s1.length && !s2.length) return 65536;

  const s1Dict = makeDict(s1);
  const s2Dict = makeDict(s2);

  let unionDict = {... s1Dict, ...s2Dict};
  let inter = 0;
  for (const [ s1K, s1V ] of Object.entries(s1Dict)) {
      for (const [ s2K, s2V ] of Object.entries(s2Dict)) {
          if (s1K === s2K) {
              inter += Math.min(s1V, s2V);
              unionDict[s1K] = Math.max(s1V,s2V);
          }
      }
  }

  let union = Object.values(unionDict).reduce((acml, v) => acml + v);
  if (!inter) return 0;
...
function splitStr(str) {
  ...
  for (let i = 0; i < str.length - 1; i++) {
      arr.push(str.slice(i, i + 2).toUpperCase());
  }

  const alpah = /^[A-Za-z]+$/;
  return arr.filter(v => v.match(alpah)).sort();    
}
