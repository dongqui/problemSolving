function solution(n, build_frame) {
  ...
  for (let i = 0; i <= n; i++) {
    t.push(new Array(n + 1).fill(EMPTY));
  }

  const isValid = ([x, _y, a]) => {
    const y = n - _y;
    let check;
    if (a === 0) {
      // 기둥 검증
      check = (
        ...
        || (x > 0 && t[y][x - 1] >= B)
        || (t[y][x] >= B)
        || (t[y + 1][x] === P || t[y + 1][x] === (P + B))
      );
    } else {
      // 보 검증
      check = (
        (t[y + 1][x] === P || t[y + 1][x] === P + B)
        || (t[y + 1][x + 1] === P || t[y + 1][x + 1] === P + B)
        || (
          (x > 0 && t[y][x - 1] >= B)
          && (x < n && t[y][x + 1] >= B)
        )
  ...
  for (const [x, _y, a, b] of build_frame) {
    const y = n - _y;
    ...
    if (b) {
      ...
      answer.splice(
        answer.findIndex(([i, j, k]) => i === x && j === _y && k === a),
        1
      );
      t[y][x] -= (a + 1);
    }
    ...
    // 조건에 안맞으면 복원
    let validation = answer.every(isValid);
    if (validation === false) {
      if (b) {
        answer.pop();
        t[y][x] -= (a + 1);
    ...
    //console.log(`(${x}, ${_y}) 에 ${a ? '보' : '기둥'} ${b ? '건설' : '제거'} ${validation ? '성공' : '실패'}`);
  }
