function solution(nodeinfo) {
  const answer = [];
  nodeinfo.forEach((v, idx) => v.push(idx + 1));
  nodeinfo.sort((a, b) => {
  ...
  postorder() {
      ...
      while (stack.length) {
          ...
          if (!cur.left && !cur.right) {
              postorderArr.push(stack.pop().idx);
              continue;
          }
          if (visit.includes(cur)) {
              postorderArr.push(stack.pop().idx);
              continue;
          }
          cur.right && stack.push(cur.right);
